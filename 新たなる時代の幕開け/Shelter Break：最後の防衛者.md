...
- 敵撃破で経験値を獲得し、必要EXPを超えるとレベル+1
- レベルが上がるほど必要EXPは増加

### 経験値システム（追加）

- 敵の進化段階に応じて獲得経験値が変動
  - 進化0: 3 / 進化1: 6 / 進化2: 9 / 進化3: 14
- 必要経験値はレベルが上がるほど増加
  - 初期必要EXP: 28
  - 以降: `前回 ×1.35 + 2` の増加
...
- 敵撃破数が一定数に達すると強化回数をストック
- レベルアップ回数も強化回数としてストック
- 強化画面は **5レベル分たまった時**にまとめて表示
- 5回分の強化を連続で選択できる
...
| 進化2  | 40  | 高   | 不可避のレーザー攻撃。ダメージ無しで0.5秒停止、頻度は低め |
| 進化3  | 500 | 中   | 無差別範囲攻撃               |
...
| 進化3  | 500 | 高速移動 | 数1：遠距離へ瞬間移動し、鞭による遠距離攻撃を行う。 |
